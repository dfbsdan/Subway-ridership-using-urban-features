...
def STEmbedding(SE, TE, UE, T, D, bn, bn_decay, is_training):
  '''
  ...
  '''if UE != None:
    UE = FC( # [batch_size, P + Q, N, F] -> [batch_size, P + Q, N, D]
      UE, units = [D, D], activations = [tf.nn.relu, None],
      bn = bn, bn_decay = bn_decay, is_training = is_training)
    STE = tf.add(STE, UE)'''
  return STE
...
def GMAN(P: int, Q: int, N: int, F: int, SE_D: int, T: int, L: int, K: int, d: int, bn: bool, bn_decay: float):
  '''
  P: input steps
  Q: output steps
  ...
  #UE = tf.keras.Input(shape=(2,N,UE_D), name="urban_embedding", dtype=tf.float32)
  ...
  # STE
  STE = STEmbedding(SE, TE, None, T, D, bn, bn_decay, is_training)
