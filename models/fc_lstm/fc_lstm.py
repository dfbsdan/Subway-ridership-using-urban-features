...
max_epoch = 1000
offs_months = 7
save_path = '/mnt/Storage/Documents/thesis/v2/models/fc_lstm/fc_lstm.model'
batch_size = 16
decay_rate = 0.7
lstm_units = 256
random.seed(1)
...
class FC_LSTM(tf.keras.Model):
  def __init__(self, units: int, Q: int, N: int):
    super().__init__()
    self.Q = Q
    self.lstm_cell = tf.keras.layers.LSTMCell(units)
    self.lstm_rnn = tf.keras.layers.RNN(self.lstm_cell, return_state=True)
    self.dense = tf.keras.layers.Dense(N*2, activation='relu')
    
  def call(self, x: tf.Tensor, training=None):    
    y, *state = self.lstm_rnn(x) # (batch, 7*H, N*2) => (batch, lstm_units)
    y = self.dense(y) # (batch, N*2)
    y_Q = []
    y_Q.append(y)
    for _ in range(1, self.Q):
      y, state = self.lstm_cell(y, states=state, training=training)
      y = self.dense(y)
      y_Q.append(y)
    y_Q = tf.stack(y_Q) # (Q, batch, N*2)
    y_Q = tf.transpose(y_Q, [1, 0, 2]) # (batch, Q, N*2)
    return y_Q
...
xy = np.stack(xy) # (M-offs_monts, 2, 7*H, N*2)
...
for _ in range(5):
  ...
  model = FC_LSTM(lstm_units, 7*H, N)
